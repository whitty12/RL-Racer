...
STATE_W = 8  # less than Atari 160x192
STATE_H = 8
VIDEO_W = 600
VIDEO_H = 400
WINDOW_W = 1200
...
class CarRacing(gym.Env):
    ...
    def step(self, action):
        ...
        step_reward = learner.reward(self.state, action)
        ...
        if action is not None:  # First step without action, called from reset()
            ...
            self.reward = step_reward + self.prev_reward
            ...
            # If we've left the playing field, end the episode
...
if __name__ == "__main__":
    ...
    env = CarRacing()
    learner = comparisonracer.RANDOM_Learner(alpha=10 ** -3, epsilon=.1)
    env.render()
    record_video = False
    if record_video:
        env.monitor.start('/tmp/video-test', force=True)
    episode_num = 0
    file = open("comparisontimevsreward.csv", 'w')
    file.write("Episode,Max Reward,Time\n")
    while True:
        env.reset()
        total_reward = 0.0
        max_reward = 0.0
        ...
        print(learner.q.values())
        start = time.time()
        while True:
            # Take a step
            a = learner.step(env.state)
            s, r, done, info = env.step(a)
            learner.update(s, r, done)
            total_reward += r
            ...
            # if steps != 0:
            #     plt.imshow(learner.cropped_state(s))
            #     plt.show()
            steps += 1
            if not record_video:  # Faster, but you can as well call env.render() every time to play full window.
                env.render()
            if done or restart or steps > 10000 or total_reward < -100:
                break
        elapsed = (time.time() - start)
        file.write(
            str(episode_num) + "," + str(max_reward) + "," + time.strftime("%H:%M:%S", time.gmtime(elapsed)) + "\n")
        file.flush()
        episode_num += 1
    env.close()
