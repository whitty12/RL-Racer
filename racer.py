...
if __name__=="__main__":
    ...
    env = CarRacing()
    learner = sarsalearning.SARSA_Learner(alpha = 10**-3, epsilon = .1)
    env.render()
    record_video = False
    if record_video:
        env.monitor.start('/tmp/video-test', force=True)
    episode_num = 0
    file = open("episoderewards.csv", 'w')
    ...
    while True:
        ...
        file.write(str(episode_num) + "," + str(max_reward) + "," + time.strftime("%H:%M:%S", time.gmtime(elapsed)) + "\n")
        file.flush()
        episode_num += 1
